...
num = 0;
observer = ["rumi","kanekosensei"]
#obs = 'kanekosensei'
emotions =  ["anxious","relax"]
n = 4;

list1 = [[1,4]];
list2 = [[0,6],[0,1],[1,3],[0,5],[0,3],[1,7]]


f = open('order_2-2-1.txt', 'w')
...
for obs in observer:
    ...
    for i in range (0,len(list)): 
        ...
        while(len(keys)==0):
                    ...
                    message =  visual.TextStim(mywin, pos = [-0.05,-2.5], height = 1.5, color = (0,0,0),text = '1 or 5')
                    message_pause.draw()
                    evaluation.draw()
                    message.draw()
                    mywin.flip()
